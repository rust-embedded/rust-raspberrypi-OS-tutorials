...
El primer paso consiste en añadir al `driver subsystem` a nuestro núcleo (kernel). El código correspondiente estará en `src/driver.rs`. El subsistema añade `interface::DeviceDriver`, una trait común que cada driver necesitará implementar y que es conocida por el núcleo (kernel). La instancia `DRIVER_MANAGER` (de tipo `DriverManager`) que se instancia en el mismo archivo sirve como la entidad central que se puede llamar para administrar todos los controladores de dispositivos en el núcleo del sistema operativo.
...
En `src/bsp/raspberrypi/driver.rs`, la función `init()` se encarga de registrar el `UART` y los controladores `GPIO`. Por lo que es importante que se realice el orden correcto:

1. Inicializar el subsistema del controlador BSP

2. Y solo si el primer paso se completó, se realiza la llamada a `driver_manager()`, como lo muestra el siguiente fragmento de `main.rs`:
...
3. Copia los siguientes archivos del  [Repositorio de firmware para Raspberry Pi](https://github.com/raspberrypi/firmware/tree/master/boot) a la tarjeta SD:
...
3. Copia los siguientes archivos del [Repositorio de fimware para Raspberry Pi](https://github.com/raspberrypi/firmware/tree/master/boot) a la tarjeta SD:
...
7. Conecta la USB serial a tu máquina princia.
   - Diagrama de conexión en [top-level README](../README.md#-usb-serial-output).
